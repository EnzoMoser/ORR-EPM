...
def get_scr(img, roi_rad, adi_rad, tum_rad=1.5):
    ...
    img_for_iqm = np.abs(img)

    # Get ROI for the breast region
    adi_roi = get_roi(roi_rho=adi_rad,
                      m_size=np.size(img, axis=0),
                      arr_rho=roi_rad)

    # Remove responses outside of the breast
    img_for_iqm[~adi_roi] = 0

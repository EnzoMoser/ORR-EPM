...
if __name__ == "__main__":
    ...
    s11 = s11[0:8, :, :]  # Only keep eight scans and ONLY MEASURE USING TWO ANTENNAS.
    ...
    labels = np.array( ["Signal", "Adi-Cropped Signal"] )
    data_channel1 = np.array( [  s11[0, :, 0] , crop[:, 0] ] )
    channel1_magnitude = control.mag2db(abs(data_channel1))
    channel1_phase = np.unwrap(np.angle(data_channel1))
    plt.subplot(2, 1, 1)
    plt.plot(recon_fs, np.transpose(channel1_magnitude), label=labels)
    plt.xlabel('Frequency (Hz)')
    plt.ylabel('Magnitude (dB)')
    plt.legend()

    plt.subplot(2, 1, 2)
    plt.plot(recon_fs, np.transpose(channel1_phase), label=labels)
    plt.xlabel('Frequency (Hz)')
